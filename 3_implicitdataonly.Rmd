---
title: "Reviewer comments_Modeling using Implicit Data Only"
...
implcit_data <- read.csv("Implicit Learning Data For Analysis.csv")
print(implcit_data)
```
#Let's center the data by substracting the mean:
```{r}
implcit_data$block_centered.c <- implcit_data$Block_number - mean(implcit_data$Block_number)
print(implcit_data)
```
#The data above is IMPLICIT data only, so let's not seperate the data into two so that we have the AV part (training data) seperated from the AO part (testing data) for two SEPPERATE models:
```{r}
implicit_data_split <- split(implcit_data, implcit_data$Part_Type)

AV_data <- implicit_data_split$`Audio+Video`
print(AV_data)

A0_data <- implicit_data_split$`Audio Only`
print(A0_data)
```
...
training_AV_model_only <- lmer(log(rt) ~ trialtype * block_centered.c + (1 + block_centered.c | participant_id), data = AV_data_seq_catch, control = lmerControl(optimizer = 'bobyqa'))
...
```
```{r}
training_AO_model_only <- lmer(log(rt) ~ trialtype * block_centered.c + (1 + block_centered.c | participant_id), data = A0_data_seq_catch, control = lmerControl(optimizer = 'bobyqa'))
...
#We now have to do post-hoc testing using the emmeans package because there is a siginificant main effect of blocks:
```{r}
library(emmeans)
