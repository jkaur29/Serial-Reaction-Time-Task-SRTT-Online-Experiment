---
title: "Reviewercomments_implicitdataonly_accuracy"
...
```{r}
implicit_data_acc <- read.csv("IL_Accuracy_data_for_analysis.csv")
print(implicit_data_acc)
```
#Let's center the data by substracting the mean:
```{r}
implicit_data_acc$block_centered.c <- implicit_data_acc$Block_number - mean(implicit_data_acc$Block_number)
print(implicit_data_acc)
```
#Let's seperate the implicit learning data above into two datasets (training - AV and testing AO) so we can add a few things before modeling them:
```{r}
implicit_data_acc_split <- split(implicit_data_acc, implicit_data_acc$Part_Type)

AV_data_acc <- implicit_data_acc_split$`Audio+Video`
print(AV_data_acc)

A0_data_acc <- implicit_data_acc_split$`Audio Only`
print(A0_data_acc)
```
...
model1_training <- lmer(accuracy_int ~ 1 + (1 | participant_id), data = AV_data_acc_seq_catch)

anova(full_model_training,model1_training)
```
```{r}
library(sjPlot)
